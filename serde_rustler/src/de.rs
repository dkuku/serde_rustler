...
impl<'de, 'a: 'de> de::Deserializer<'de> for VariantNameDeserializer<'a> {
        ...
        V: Visitor<'de>,
    {
        match self.variant.get_type() {
            TermType::Atom => {
                let string =
                    atoms::term_to_variant_string(&self.variant).or(Err(Error::InvalidVariantName))?;
                visitor.visit_string(string)
            }
            TermType::Binary => visitor.visit_string(util::term_to_str(&self.variant)?),
            TermType::Integer | TermType::Float => visitor.visit_string(util::term_to_str(&self.variant)?),
